...
void *__ezheap_malloc(size_t __size, ezheap_t *__ezheap)
{
  ...
  else if (__ezheap->heap_addresses != NULL && __ezheap->heap_addresses_len > 0)
  {
    size_t old_array_len_bytes = sizeof(void *) * __ezheap->heap_addresses_len;

    // create a new array
    void *new_heap_addresses = __ezheap_stdmalloc(old_array_len_bytes + sizeof(void *), __ezheap);
    ...
    memcpy(new_heap_addresses + old_array_len_bytes, &heap_addr, sizeof(void *));
...
void *__ezheap_malloc_errsafe(size_t __size, ezheap_t *__ezheap)
{
  ...
  if (result == NULL)
  {
    printf("[EZHEAP] __ezheap_malloc() failed: either malloc() failed, or bad heap_addresses\n");
    abort();
  }
  return result;
}
